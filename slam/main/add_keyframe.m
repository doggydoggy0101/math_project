...
function [mapPoints, vSetKeyFrames] = add_keyframe(mapPoints, vSetKeyFrames,...
...
vSetKeyFrames = addView(vSetKeyFrames, viewId, cameraPose, Features=currFeatures.Features, Points=currPoints);
...
for i = 1:numel(keyFramesIndices)
    ...
    relPose = rigidtform3d(prePose.R' * cameraPose.R, (cameraPose.Translation-prePose.Translation)*prePose.R);
    
    if numel(ia) > 5
        vSetKeyFrames = addConnection(vSetKeyFrames, localKeyFrameId, viewId, relPose, Matches=[index2d(ia),featureIndices(ib)]);
    end
end
